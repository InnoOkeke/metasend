...
export type RampProvider = "coinbase" | "moonpay" | "paycrest";
...
export async function getAvailableProviders(type: RampType): Promise<RampProvider[]> {
  const providers: RampProvider[] = ["coinbase", "moonpay"];
...
/**
 * Build MoonPay URL
 * Docs: https://dev.moonpay.com/docs/on-ramp
 */
export function buildMoonPayUrl(config: RampConfig): string {
  ...
  const params = new URLSearchParams({
    ...
    currencyCode: config.assetSymbol?.toLowerCase() ?? "usdc",
  });
  ...
  if (config.destinationCurrency) {
    params.append("quoteCurrencyCode", config.destinationCurrency.toLowerCase());
  }
...
export function buildPaycrestUrl(config: RampConfig): string {
  const params = new URLSearchParams({
    ...
    walletAddress: config.walletAddress,
    asset: config.assetSymbol?.toUpperCase() ?? "USDC",
    network: config.destinationNetwork ?? "BASE",
  });
...
export function buildRampUrl(config: RampConfig): string {
  switch (config.provider) {
    case "coinbase":
      return buildCoinbaseRampUrl(config);
    case "moonpay":
