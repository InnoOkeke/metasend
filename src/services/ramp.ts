...
export function buildMoonPayUrl(config: RampConfig): string {
  const params = new URLSearchParams({
    ...
    // Use USDC_BASE for Base network, not USDC (which is Ethereum)
    currencyCode: config.assetSymbol === "USDC" ? "usdc_base" : (config.assetSymbol?.toLowerCase() ?? "usdc"),
  });
...
export function buildTransakUrl(config: RampConfig): string {
  ...
  const params = new URLSearchParams({
    ...
    walletAddress: config.walletAddress,
    cryptoCurrencyCode: config.assetSymbol?.toUpperCase() ?? "USDC",
